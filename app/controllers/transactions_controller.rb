class TransactionsController < ApplicationController
  ...
  def index
    ...
    if !session[:user_id] && params[:filter] == "pickups"
      @transactions = Transaction.where("DATE(transactions.scheduledPickupStartDT) = ?", Date.today)
    elsif !session[:user_id] && params[:filter] == "recharges"
      @transactions = Transaction.where("DATE(transactions.rechargeDueDT) = ?", Date.today)
